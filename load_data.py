...
im = Image.open('cup.jpg').convert('RGB')
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        tx = ((-target_x+0.5)*2).cuda()
        ty = ((-target_y+0.5)*2).cuda()
...
class InriaDataset(Dataset):
    ...
    def __init__(self, img_dir, lab_dir, max_lab, imgsize, shuffle=True):
        ...
        self.img_names = fnmatch.filter(os.listdir(img_dir), '*.png')
        ...
        for img_name in self.img_names:
            ...
            lab_path = os.path.join(self.lab_dir, img_name).replace('.png', '.txt')
    ...
    def __getitem__(self, idx):
        ...
        lab_path = os.path.join(self.lab_dir, self.img_names[idx]).replace('.png', '.txt')
...
if __name__ == '__main__':
    ...
    cfgfile = "cfg/yolov.cfg"
    weightfile = "weights/yolov.weights"
    ...
    for i_batch, (img_batch, lab_batch) in enumerate(test_loader):
        ...
        adv_patch = Image.open('cup.jpg').convert('RGB')
